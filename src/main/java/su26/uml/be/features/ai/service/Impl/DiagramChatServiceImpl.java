...
import su26.uml.be.features.ai.dto.AiNodeDto;
import su26.uml.be.features.ai.dto.AiResponseKind;
import su26.uml.be.common.response.ApiResponse;
import su26.uml.be.features.ai.dto.AnythingLlmChatResponse;
...
import su26.uml.be.features.ai.entity.AiSourceDocument;
...
import su26.uml.be.features.ai.repository.AiChatMessageRepository;
import su26.uml.be.features.ai.repository.AiChatSessionRepository;
import su26.uml.be.features.user.repository.UserRepository;
import su26.uml.be.features.ai.service.SystemConfigCacheService;
import su26.uml.be.features.admin.service.ActivityTrackerService;
import su26.uml.be.features.ai.service.DiagramChatService;
...
public class DiagramChatServiceImpl implements DiagramChatService {
    ...
    public ApiResponse<PagedResponse<ChatSessionResponse>> getSessions(String email, Pageable pageable) {
        ...
        Page<ChatSessionResponse> page = chatSessionRepository
                .findByUserIdOrderByUpdatedAtDesc(userId, pageable)
                .map(this::mapSessionResponse);

        return ApiResponse.success("Get chat sessions successfully", PagedResponse.from(page));
    }
    ...
    public ApiResponse<PagedResponse<DiagramChatHistoryResponse.MessageItem>> getHistory(
        ...
        Page<DiagramChatHistoryResponse.MessageItem> page = chatMessageRepository
                .findByChatSessionIdOrderByCreatedAtAsc(session.getId(), pageable)
                .map(message -> {
        ...
        return ApiResponse.success("Get chat history successfully", PagedResponse.from(page));
    }
